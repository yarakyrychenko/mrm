...
if st.session_state.changed:
    ...
    if len(df.MeasureName) == 0:
        ...
        for MeasureName in pd.unique(df.MeasureName):
            i += 1
            st.markdown(f"{i}. {str(df[df.MeasureName==MeasureName].Year.value)}. {df[df.MeasureName==MeasureName].MeasureName}. Link to Paper: {df[df.MeasureName==MeasureName].LinkPaper}. Link to Measure: {df[df.MeasureName==MeasureName].LinkMeasure}") 
